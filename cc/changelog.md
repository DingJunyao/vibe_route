...
- 修复「轨迹设为印尼后详情页仍显示纯文本编号」+ 道路标志生成入口印尼支持 + 地区代码表对齐 gpxutil（后端 pytest **108 passed**，2026-09-11）：
  - **根因**：图标渲染的权威是**点级** `track_points.region`，而实测问题轨迹（track 85）11729 个点**全部为 `'cn'`**（印尼数据由导入路径写入 `province_id`/`road_name_id` 列，`region` 列缺失落模型默认 `'cn'`）→ `_build_region_tree` 出的节点全 `cn` → 前端走国标 `parseRoadNumber()`，纯数字编号全返回 `null` → 回退文本。编辑对话框此前明确「已有轨迹点不受影响」，轨迹级地区对已有点**永远不生效**；增量填充只处理 `province` 为空的点（本轨迹仅 43/11729），全量重填约 40 分钟，补救路径不通
  - **编辑同步开关**：`TrackUpdate.sync_points_region`（默认 False）+ 编辑对话框「同时更新已有轨迹点地区（共 N 个点）」（默认勾选）。**仅当 region 实际变化且勾选时**才批量 `UPDATE track_points`（改名不重写上万行）；`track_service.update` 须先 `pop('sync_points_region')` 再 setattr（非 Track 列，直接 setattr 抛 AttributeError）。点级权威语义不变，未勾选时行为与修复前逐字一致（`test_region_propagation` 保持绿）
  - **地区代码表对齐 gpxutil `a31e1a8`**：`INDONESIA_PROVINCE_CODE_MAP` 由 38 省 × 双键 = 76 条（ISO 3166-2:ID，东爪哇 `'35'`）换为 **34 省 × 三语键 = 102 条**（印尼语/中文/英文）的交通部法规 kode wilayah **1-34 序号**（东爪哇 `'16'`），并剔除无码的 2022 巴布亚四省；删除零使用的 `_PROVINCE_ID_BY_CODE`。**旧缓存/旧数据的 `'35'` 语义全部作废**
  - **县市码**：`16.17-024` → 大字 `024`、色带 `PROVINSI 16.17`（`省码.县序号` 形式亦须命中值集）
  - **`force_tol`**：`parse_indonesia_road_num` / `generate_road_sign` / `RoadSignRequest` 新增，强制 1-2 位编号按 TOL 解析（3 位恒 PROVINSI，不受影响）
  - **`'-024'` 回退语义对齐 gpxutil**：删掉「空前缀即无法识别」的守卫，改为一律回退 `province_texts` 查表（唯一与旧实现不同的语义变化；DB 中无 `'-024'` 数据，无实际影响）。注：旧数据 `'35-024'` 未命中新值集 → 同样回退省名查表，**得 `'16'` 而非降级无省码**（实测已覆盖于 `test_legacy_bps_prefixed_code_falls_back`）
  - **生成入口印尼模式**：生成页与主页内嵌对话框均加地区切换 + id 字段（编号 / 中文路名 / 印尼语路名 / 省份文本 / 收费公路）；生成页另加 4 条印尼示例与历史回填。主页的国标字体检查 `areFontsConfigured()` **移入 `generateRoadSign` 且仅 cn 执行**（印尼盾牌用 `data/fonts/` 的 Clearview 字体，与 A/B/C 配置无关，原先会在对话框入口就误挡）
...
- **cn 轨回归风险（低概率，方向是「显示更多信息」而非更少）**：区域树的 `city`/`district`/`road_name` 现走 **zh → id → en** 回退链（`track_service.py:1504-1506`），故 cn 轨在「zh 某级为空、en 该级有值」时会**新出现一个英文名节点**，而合并前该点会并进上层节点。geocoding 三语取自同一轮请求、同一 `admin` 层级，presence 应一致 → 实际概率很低；但相对既有 cn 数据确属显示差异，记录在案
- 修复既有缺陷：**导出的 CSV 首行曾是仅含 BOM 的空行**，导致自家「导出 → 再导入」**静默失效**。成因：`export_points_to_csv` 以 `csv_lines.append(BOM)` 与表头分列两行再 `"\n".join` → BOM 独占首行；导入侧 `decode('utf-8-sig')` 去掉 BOM 后首行成空行，`csv.DictReader` 把它当表头 → `fieldnames` 为 `[]`、每行键为 `None` → `row.get("index")` 恒 `None` → **匹配 0 个点、返回 `matched_by: none`，全程不报错**。该缺陷自 `40aabe4` 起存在，与多语言工作无关，但本计划要求的「导出 → 再导入」闭环在未修时不可能通过，故一并修复（BOM 并入首字段）。**结论：vibe_route 自家导出的 CSV 在这次之前无法被自家导入器正确读回**；修复后闭环成立。另注：行分隔符仍是 `\n` 而非 RFC 4180 的 `\r\n`（既有，未改，导入侧不受影响）
- 发现但**未修**的既有缺陷：**CSV 导入（创建路径）只认斜杠日期，连字符日期会静默丢时间**。`_create_from_csv_project_format` 解析 `time_date + time_time` 时**只试 `%Y/%m/%d %H:%M:%S` 一种格式**，而更新/地理编码路径试 4 种、含 `%Y-%m-%d`。后果：`time_date` 写成 `2026-09-01` 的 CSV **导入（创建）成功但 `start_time`/`end_time` 双双为 null**（仅打一条 `logger.warning`），轨迹列表与区域树时间显示为空；同一文件走「更新」路径则正常。属独立议题、超本次范围；若要修，最小改动是把那一行单格式 `strptime` 换成与更新路径相同的格式循环
- 后端 pytest 全量 **84 passed**（20 warnings，2026-09-10 实测）。本次新建 5 个测试文件：`test_indonesia_road.py`、`test_indonesia_shield.py`、`test_road_sign_region.py`、`test_nominatim_region.py`、`test_region_propagation.py`
- UI 端到端冒烟通过（2026-09-10，五步全过、**无代码改动**）：① 上传印尼 CSV（12 点 / 26 列，region=id，斜杠日期正确解析）；② 区域树 `2 省级 / 2 地级 / 2 县级`，印尼段与 cn 段并列成两个根，3 个六边形盾牌（`PROVINSI 35` 蓝 / `TOL 3` 红 / `NASIONAL 3` 红）**用祖先链回溯逐一核对**无错挂，tooltip 为原生 `title`（三语节点全有、`road_name_zh` 为空的节点正确降为双语、cn 单语节点无 tooltip）；③ 编辑对话框回显「印尼」，切「中国」保存后 `tracks.region` 变更而 **`track_points.region` 未变**（点级权威未被覆盖），`road_sign_cache` 出现 `id` 3 条（缓存键含 region 的实证）；④ 导出（`text/csv`、4016 B、13 行）表头 30 列、首行即表头无 BOM 空行，再导入 **12/12 全匹配**，两次 `/regions` 响应 `JSON.stringify` 逐字节相同（3116 字符）；⑤ cn 轨迹回归：11627 点轨迹 `3 省级 / 9 地级 / 16 县级`、23 个国标盾牌配色与并列编号正常。**冒烟环境伪影（非应用缺陷）**：操作成功后对话框延迟很久甚至不关闭——导出触发下载使 Edge 抢走前台，后台标签的 `requestAnimationFrame` 被浏览器停止，而 `el-dialog` 关闭走 Vue `<Transition>`（leave 第一步即 double rAF）卡住；把 rAF 临时 polyfill 成 `setTimeout` 后对话框 3 秒内正常关闭，组件状态实测均正确
- **前端无 TS 门禁**：`vue-tsc` 在本机**完全不可用**（`vue-tsc@1.8.27` × `typescript@5.9.3`，`npx vue-tsc --version` 自身即崩 `Search string not found: "/supportedTSExtensions = .*(?=;)/"`；lockfile 固化 5.9.3 故 `npm ci` 修不好；`vue-tsc@2` 路线同样堵死——本地 TS 5.9.3 未导出 `./lib/tsc`）。故 `npm run build:check` 不可用，前端验收改用项目自身文档标准 `npm run build` + 人工定点核对（新增必填字段 → 找对象字面量构造点；改签名 → 找全部调用点；删 import → 找别的使用点）。**且仓库 TS 基线本就不干净**：用 `node node_modules/typescript/bin/tsc` 实测出 3 条真错（`src/api/request.ts` 的 `config.skipProgress` ×2、`src/api/roadSign.ts` 的 `skipAuth`；`src/` 下无任何 `declare module 'axios'` 增补）——修好 vue-tsc 的那天仓库不会立刻全绿，「无新增类型错误」只能相对这个脏基线判定。**已上报开发者待裁决**（是否单开任务修 vue-tsc / 补 axios 类型增补），**尚未裁决、未处理**
