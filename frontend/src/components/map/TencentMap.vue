...
<script lang="ts">
// 腾讯地图日志收集器（全局函数，供类和 setup 使用）
const tencentMapLogs: string[] = []
const MAX_TENCENT_LOGS = 100

function addTencentLog(category: string, message: string, data?: any) {
  const timestamp = new Date().toLocaleTimeString('zh-CN')
  const logEntry = `[${timestamp}][Tencent-${category}] ${message}${data ? ' ' + JSON.stringify(data) : ''}`
  tencentMapLogs.push(logEntry)
  if (tencentMapLogs.length > MAX_TENCENT_LOGS) {
    tencentMapLogs.shift()
  }
}

// 暴露给全局
;(window as any).__tencentMapLogs = tencentMapLogs

// 更新全局的复制日志函数（包括腾讯地图日志）
;(window as any).__copyAllLogs = () => {
  const animationLogs = (window as any).__animationLogs || []
  const allLogs = [...animationLogs, ...tencentMapLogs].join('\n')
...
class AnimationDOMOverlay {
  ...
  private lastGeoPos: { lat: number; lng: number } | null = null // 缓存上次地理坐标
  ...
  private addToMap() {
    ...
    addTencentLog('addToMap', 'called', {
      hasMap: !!this.map,
      mapDivExists: !!mapDiv,
      mapDivTag: mapDiv?.tagName,
      mapDivId: mapDiv?.id,
      mapDivClass: mapDiv?.className,
      elementInDocument: document.body.contains(this.element)
    })

    if (mapDiv) {
      // 检查容器的定位属性
      const containerStyle = window.getComputedStyle(mapDiv)
      addTencentLog('addToMap', 'container styles', {
      ...
      // 查找正确的容器 - 需要是具有 position: relative/absolute/fixed 的容器
      // 避免添加到可能被其他样式影响的容器中
      let targetContainer = mapDiv as HTMLElement

      // 尝试查找第一个 canvas 的直接父级（通常是地图渲染层）
      const canvas = mapDiv.querySelector('canvas')
      if (canvas && canvas.parentElement) {
        const canvasParent = canvas.parentElement
        const parentStyle = window.getComputedStyle(canvasParent)
        addTencentLog('addToMap', 'canvas parent found', {
        ...
        // 如果 canvas 父级有定位属性，使用它作为目标容器
        if (parentStyle.position !== 'static') {
          targetContainer = canvasParent as HTMLElement
          addTencentLog('addToMap', 'using canvas parent as target container')
        }
      }

      this.mapContainer = targetContainer
      this.mapContainer.appendChild(this.element)

      addTencentLog('addToMap', 'element appended', {
        mapContainerClass: this.mapContainer.className,
        mapContainerTag: this.mapContainer.tagName,
        elementParent: this.element.parentElement?.className || this.element.parentElement?.tagName || 'none',
        elementInContainer: this.mapContainer.contains(this.element)
      })
      ...
      addTencentLog('addToMap', 'event listeners added')
    } else {
      addTencentLog('addToMap', 'FAILED - mapDiv not found')
    }
  }
  ...
  setPosition(lat: number, lng: number) {
    ...
    // 检查地理坐标是否发生了变化
    // 如果变化超过阈值，清除 lastPixelPos 以强制更新
    const geoChanged = !this.lastGeoPos ||
      Math.abs(lat - this.lastGeoPos.lat) > 0.0001 ||
      Math.abs(lng - this.lastGeoPos.lng) > 0.0001

    if (geoChanged && this.lastPixelPos) {
      this.lastPixelPos = null
      addTencentLog('setPosition', 'geographic position changed, forcing update', {
        oldPos: this.lastGeoPos,
        newPos: { lat, lng }
      })
    }

    this.lastGeoPos = { lat, lng }
    this.updatePosition()
  }
  ...
  private updatePosition() {
    if (!this.position || !this.map) {
      addTencentLog('updatePosition', 'early return - missing data', {
    ...
    // 记录 projectToContainer 返回的原始值
    addTencentLog('updatePosition', 'projectToContainer result', {
      latLng: `${this.position.lat.toFixed(6)}, ${this.position.lng.toFixed(6)}`,
      pointPixelX: pointPixel.x,
      pointPixelY: pointPixel.y,
      lastPixelPos: this.lastPixelPos
    })

    if (!pointPixel) {
      addTencentLog('updatePosition', 'pointPixel is null', {
        position: `${this.position.lat.toFixed(6)}, ${this.position.lng.toFixed(6)}`
      })
      return
    }
    ...
    this.lastPixelPos = { x: newLeft, y: newTop }
      ...
      setTimeout(() => {
        ...
        addTencentLog('updatePosition', 'after transform update', {
          elementRect: {
            x: rect.x,
            y: rect.y,
            width: rect.width,
          ...
          innerHtml: this.innerElement.innerHTML.substring(0, 50),
          innerElementChildren: this.innerElement.children.length
        })
      }, 0)

      addTencentLog('updatePosition', 'transform updated', {
        transform: this.element.style.transform,
        newLeft,
        newTop,
        markerSize: `${width}x${height}`,
        wasNull: !this.lastPixelPos
      })
  }
  ...
  private updateContent() {
    ...
    addTencentLog('updateContent', 'called', {
      style: this.style,
      width,
      height,
      innerElementExists: !!this.innerElement,
      elementExists: !!this.element,
      innerElementInDocument: document.body.contains(this.innerElement),
      elementInDocument: document.body.contains(this.element)
    })
    ...
    // 延迟检查内部元素的实际状态
    setTimeout(() => {
      const img = this.innerElement.querySelector('img')
      const svg = this.innerElement.querySelector('svg')
      const innerHtml = this.innerElement.innerHTML
      addTencentLog('updateContent', 'after update', {
        innerHtml: innerHtml.substring(0, 80),
        innerElementDisplay: this.innerElement.style.display,
        innerElementWidth: this.innerElement.style.width,
        innerElementHeight: this.innerElement.style.height,
        innerElementChildren: this.innerElement.children.length,
        imgExists: !!img,
        imgSrc: img?.src || 'none',
        imgDisplay: img ? window.getComputedStyle(img).display : 'none',
        svgExists: !!svg
      })
    }, 0)
  }
...
const animationAdapter: AnimationMapAdapter = {
  ...
  addTencentLog('setMarkerPosition', 'called', { position: `${position.lat.toFixed(6)}, ${position.lng.toFixed(6)}`, style })
  addTencentLog('setMarkerPosition', 'status', {
    TMapInstance: !!TMapInstance,
    animationMarker: !!animationMarker
  })

  if (!TMapInstance) {
    addTencentLog('setMarkerPosition', 'TMapInstance not available')
    return
  }
  ...
  try {
    ...
    const TMap = (window as any).TMap
    const positionLatLng = new TMap.LatLng(position.lat, position.lng)
    const isInBounds = bounds ? bounds.contains(positionLatLng) : false

    addTencentLog('setMarkerPosition', 'Map state', {
      zoom,
      center: center ? `${center.lng.toFixed(4)}, ${center.lat.toFixed(4)}` : 'null',
      bounds: bounds ? `sw:(${bounds.getSouthWest().lng.toFixed(4)}, ${bounds.getSouthWest().lat.toFixed(4)}) ne:(${bounds.getNorthEast().lng.toFixed(4)}, ${bounds.getNorthEast().lat.toFixed(4)})` : 'null',
      positionInBounds: isInBounds,
      containerSize: container ? `${container.clientWidth}x${container.clientHeight}` : 'null'
    })
  } catch (e) {
    addTencentLog('setMarkerPosition', 'Map state check error', { error: String(e) })
  }
  ...
  if (!animationMarker) {
    ...
    setTimeout(() => {
      if (!animationMarker && TMapInstance) {
        addTencentLog('setMarkerPosition', 'Creating new animationMarker')
        ...
        setTimeout(() => {
          if (animationMarker) {
            addTencentLog('setMarkerPosition', 'Animation marker created successfully')
            // 检查元素是否在 DOM 中
            const element = animationMarker as any
            const domElement = element.element
            if (domElement) {
              const parent = domElement.parentElement
              const computedStyle = window.getComputedStyle(domElement)
              addTencentLog('setMarkerPosition', 'Marker DOM state', {
                inDocument: document.body.contains(domElement),
        ...
        addTencentLog('setMarkerPosition', 'Still cannot create animationMarker', {
          TMapInstance: !!TMapInstance,
          animationMarker: !!animationMarker
        })
      }
    ...
    // 更新位置
    addTencentLog('setMarkerPosition', 'Updating existing animationMarker')
    animationMarker.setPosition(position.lat, position.lng)
  ...
  setAnimationPlaying(playing) {
    ...
    if (playing) {
      ...
      if (points.length > 1) {
        fullTrackPolyline = new TMap.MultiPolyline({
          geometries: [{
            id: 'full-track',
            styleId: 'full-style',
          ...
          styles: {
            'full-style': new TMap.PolylineStyle({
              color: '#c0c4cc',
              width: 5,
              borderWidth: 0,
            }),
        ...
        fullTrackPolyline.setMap(TMapInstance)
      }
    } else {
      // 播放停止：清除完整灰色轨迹
      if (fullTrackPolyline) {
  ...
  if (bounds.length > 0 && !props.disablePointHover) {
    try {
      ...
      if (isMobile) {
        ...
        addTencentLog('drawTracks', 'Mobile fitBounds', {
          containerWidth,
          containerHeight,
          padding,
          paddingPercent: '10%'
        ...
        padding = 100
        addTencentLog('drawTracks', 'Desktop fitBounds', {
          containerWidth,
          containerHeight,
          padding
        })
      ...
      addTencentLog('drawTracks', 'fitBounds error', { error: e })
    }
  }
